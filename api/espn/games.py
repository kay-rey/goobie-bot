...
async def get_dodgers_next_game():
    ...
    try:
        ...
        response = requests.get(url, params=params, timeout=10)
        logger.info(f"ESPN MLB API response status: {response.status_code}")
...
async def get_rams_next_game():
    ...
    try:
        ...
        response = requests.get(url, params=params, timeout=10)
        logger.info(f"ESPN NFL API response status: {response.status_code}")

        if response.status_code == 200:
            data = response.json()
            logger.info(f"ESPN NFL API data keys: {list(data.keys())}")
            logger.info(f"ESPN NFL API items count: {len(data.get('items', []))}")
